...
import {
  ...
  type WorkspaceSearchMigrationPlanningSourceArtifactAuthority,
  type WorkspaceSearchMigrationPlanningSourceArtifactEncodedSegment,
  type WorkspaceSearchMigrationPlanningSourceArtifactPage,
  type WorkspaceSearchMigrationPlanningSourceArtifactSegment,
  type WorkspaceSearchMigrationPlanningSourceArtifactTableIncarnation,
...
/**
 * Narrow S3 command transport used by the immutable source-artifact adapter.
 */
export interface WorkspaceSearchMigrationSourceArtifactAwsTransport {
  /**
...
/**
 * Dependencies that bind one source-artifact port to measured AWS resources.
 */
export type CreateWorkspaceSearchMigrationSourceArtifactAwsPortInput = {
  ...
  readonly transport: WorkspaceSearchMigrationSourceArtifactAwsTransport
}

/**
 * Common page identity required to verify stored source-artifact segments.
 */
export type WorkspaceSearchMigrationPlanningSourceArtifactExpectedPageContext = {
  ...
  /** Exact measured physical source-table incarnation. */
...
/**
 * Input for storing one complete lossless planning source page.
 */
export type WriteWorkspaceSearchMigrationPlanningSourceArtifactPageInput = {
...
/**
 * Input for reading and reconstructing one stored planning source page.
 */
export type ReadWorkspaceSearchMigrationPlanningSourceArtifactPageInput = {
  /** Exact expected page identity without raw source items or a cursor. */
  readonly expectedPage:
...
/**
 * Immutable lossless planning-source storage operations.
 */
export interface WorkspaceSearchMigrationSourceArtifactAwsPort {
  /**
   * Stores every deterministic segment of one complete source page.
   *
   * @param input - Complete expected page including exact raw source items.
   * @returns Ordered exact S3 object-version references.
   */
  writePlanningSourceArtifactPage(
    input: WriteWorkspaceSearchMigrationPlanningSourceArtifactPageInput,
  ): Promise<
    readonly WorkspaceSearchMigrationPlanningSourceArtifactReference[]
  >

  /**
   * Reads exact versions and reconstructs one complete source page.
   *
   * @param input - Expected common page identity and ordered S3 references.
   * @returns Strictly parsed complete lossless source page.
   */
  readPlanningSourceArtifactPage(
    input: ReadWorkspaceSearchMigrationPlanningSourceArtifactPageInput,
  ): Promise<WorkspaceSearchMigrationPlanningSourceArtifactPage>
}

/**
 * Failure codes deliberately emitted by the private S3 adapter.
 */
type SourceArtifactAwsFailureCode =
 ...
 */
type SourceArtifactAwsErrorClassificationInput =
 ...
 */
class SourceArtifactAwsFailure extends Error {
  /** Stable secret-free code chosen inside the trusted adapter. */
  readonly code: SourceArtifactAwsFailureCode
   ...
   */
  constructor(code: SourceArtifactAwsFailureCode) {
    super(code)
    this.name = 'SourceArtifactAwsFailure'
    this.code = code
  }
}
 ...
 */
class SourceArtifactBodyTimeout extends Error {
   ...
   */
  constructor() {
    super('Source artifact body timed out.')
 ...
 */
type PreparedSourceArtifactAwsPortInput = {
 ...
 */
type PreparedSourceArtifactSegment = {
 ...
 */
type SourceArtifactObjectSnapshot = {
...
/**
 * Creates a source-artifact adapter bound to one measured configuration.
 *
 * @param input - Exact measured configuration and narrow S3 transport.
 * @returns Immutable source-artifact port.
 */
export function createAwsWorkspaceSearchMigrationSourceArtifactPort(
  input: CreateWorkspaceSearchMigrationSourceArtifactAwsPortInput,
): WorkspaceSearchMigrationSourceArtifactAwsPort {
  try {
    const prepared = prepareSourceArtifactAwsPortInput(input)
    return new AwsWorkspaceSearchMigrationSourceArtifactPort(prepared)
  } catch (error: unknown) {
    throw createSourceArtifactAwsBoundaryFailure(
      readSourceArtifactAwsFailureCode(error, 'INVALID_ARGUMENT'),
    )
  }
}

/**
 * S3 adapter for immutable lossless planning-source segments.
 */
class AwsWorkspaceSearchMigrationSourceArtifactPort
  implements WorkspaceSearchMigrationSourceArtifactAwsPort {
  /** Detached exact measured configuration. */
  ...
  /** Narrow S3 transport sharing the measured AWS session. */
  private readonly transport: WorkspaceSearchMigrationSourceArtifactAwsTransport

  /**
   * Creates an already validated source-artifact adapter.
   *
   * @param input - Detached exact configuration and transport.
   */
  constructor(input: PreparedSourceArtifactAwsPortInput) {
   ...
   * Stores every deterministic segment of one complete source page.
   *
   * @param input - Complete expected page including raw source items.
   * @returns Ordered exact immutable S3 version references.
   */
  async writePlanningSourceArtifactPage(
    input: WriteWorkspaceSearchMigrationPlanningSourceArtifactPageInput,
  ): Promise<
    readonly WorkspaceSearchMigrationPlanningSourceArtifactReference[]
  > {
    return runSourceArtifactAwsBoundary(async () => {
      const segments = prepareSourceArtifactWriteInput(
        input,
        this.configuration,
   ...
   * @returns Strictly parsed complete lossless source page.
   */
  async readPlanningSourceArtifactPage(
    input: ReadWorkspaceSearchMigrationPlanningSourceArtifactPageInput,
  ): Promise<WorkspaceSearchMigrationPlanningSourceArtifactPage> {
    return runSourceArtifactAwsBoundary(async () => {
      const prepared = prepareSourceArtifactReadInput(
        input,
        this.configuration,
        this.configurationHash,
      )
  ...
  private async putImmutableSegment(
    segment: PreparedSourceArtifactSegment,
    retryCount = 0,
  ): Promise<WorkspaceSearchMigrationPlanningSourceArtifactReference> {
    let output: PutObjectCommandOutput
    try {
      output = await this.transport.putSourceArtifact(
        createSourceArtifactPutCommand(
          this.configuration,
          segment,
        ),
      )
    } catch (error: unknown) {
      if (isSourceArtifactPreconditionFailure(error)) {
        const existing = await this.reconcileCurrentSegment(segment)
        if (existing.exists) return existing.reference
        return failSourceArtifactAws('AMBIGUOUS_OPERATION_UNRESOLVED')
      }
      if (isSourceArtifactAmbiguousWriteFailure(error)) {
        const existing = await this.reconcileCurrentSegment(segment)
        if (existing.exists) return existing.reference
        if (retryCount < sourceArtifactMaximumRetryCount) {
          return this.putImmutableSegment(segment, retryCount + 1)
        }
        return failSourceArtifactAws(
          'TRANSIENT_INFRASTRUCTURE_FAILURE',
        )
      }
      return failSourceArtifactAws('SOURCE_ARTIFACT_WRITE_FAILED')
    }

    const versionId = validateSourceArtifactPutOutput(
      output,
      segment,
      this.configuration,
    )
    let head: SourceArtifactHeadResult
    try {
      head = await this.headSegment(segment, versionId)
    } catch (error: unknown) {
      if (error instanceof SourceArtifactAwsFailure) throw error
      return failSourceArtifactAws(
        'AMBIGUOUS_OPERATION_UNRESOLVED',
      )
    }
  ...
  private async reconcileCurrentSegment(
    segment: PreparedSourceArtifactSegment,
  ): Promise<SourceArtifactHeadResult> {
    try {
  ...
  private async headSegment(
    ...
    try {
      output = await this.transport.headSourceArtifact(
      ...
      if (isSourceArtifactNotFound(error)) return { exists: false }
      throw error
    }
    const snapshot = snapshotSourceArtifactObjectOutput(output, false)
    const resolvedVersionId = validateStoredSourceArtifactObject(
      snapshot,
      segment,
      this.configuration,
      versionId,
    ...
    return {
      exists: true,
      reference: createSourceArtifactReference(
        segment,
        resolvedVersionId,
      ),
    }
  ...
  private async readVerifiedSegment(
    ...
    try {
      output = await this.transport.getSourceArtifact(
      ...
      if (isSourceArtifactRetryableFailure(error)) {
        return failSourceArtifactAws(
          'TRANSIENT_INFRASTRUCTURE_FAILURE',
        )
      }
    ...
    const snapshot = snapshotSourceArtifactObjectOutput(output, true)
    const byteLength = validateReferencedSourceArtifactObject(
      snapshot,
      reference,
      segmentIndex,
      segmentCount,
    ...
    const bytes = await readBoundedSourceArtifactBody(
      snapshot.body,
      byteLength,
    )
    if (
      digestSourceArtifactBytes(bytes) !== reference.contentDigest
    ) {
      return failSourceArtifactAws('INVALID_SOURCE_ARTIFACT')
    }
    const segment =
      parseWorkspaceSearchMigrationPlanningSourceArtifactSegment(bytes)
    if (
 ...
 */
type PreparedSourceArtifactReadInput = {
 ...
 */
function prepareSourceArtifactAwsPortInput(
  input: CreateWorkspaceSearchMigrationSourceArtifactAwsPortInput,
): PreparedSourceArtifactAwsPortInput {
  const record = requireSourceArtifactInputRecord(
    input,
    'INVALID_ARGUMENT',
  )
  requireExactSourceArtifactInputKeys(
    record,
  ...
  if (
    ...
    return failSourceArtifactAws('CONFIGURATION_HASH_MISMATCH')
  }
  requireSourceArtifactConfiguration(configuration)
  requireSourceArtifactTransport(input.transport)
  return {
    configuration,
    configurationHash,
 ...
 */
function prepareSourceArtifactWriteInput(
  input: WriteWorkspaceSearchMigrationPlanningSourceArtifactPageInput,
  configuration: WorkspaceSearchMigrationConfiguration,
  configurationHash: string,
): readonly PreparedSourceArtifactSegment[] {
  const record = requireSourceArtifactInputRecord(
    input,
    'INVALID_ARGUMENT',
  )
  ...
  return encoded.map(prepareSourceArtifactSegment)
}
 ...
 */
function prepareSourceArtifactReadInput(
  input: ReadWorkspaceSearchMigrationPlanningSourceArtifactPageInput,
  configuration: WorkspaceSearchMigrationConfiguration,
  configurationHash: string,
): PreparedSourceArtifactReadInput {
  const record = requireSourceArtifactInputRecord(
    input,
    'INVALID_ARGUMENT',
  )
  ...
  const expectedPage = snapshotExpectedSourceArtifactPageContext(
    structuredClone(input.expectedPage),
  )
  ...
  const referenceValues = readDenseSourceArtifactArray(
    input.references,
    'INVALID_SOURCE_ARTIFACT',
  )
  if (
  ...
  const references = referenceValues.map((candidate) => {
    const reference = snapshotSourceArtifactReference(candidate)
 ...
 */
function prepareSourceArtifactSegment(
  encoded: WorkspaceSearchMigrationPlanningSourceArtifactEncodedSegment,
): PreparedSourceArtifactSegment {
  try {
    ...
    if (
      !sourceArtifactBytesEqual(bytes, serialized) ||
    ...
    return {
      ...
      contentDigest,
      checksumSha256:
        createSourceArtifactChecksumSha256(bytes),
 ...
 */
function createSourceArtifactPutCommand(
  configuration: WorkspaceSearchMigrationConfiguration,
  segment: PreparedSourceArtifactSegment,
): PutObjectCommand {
  return new PutObjectCommand({
    ...
    ContentType: sourceArtifactContentType,
...
function createSourceArtifactMetadata(
  ...
  return {
    'mukuroji-kind': sourceArtifactKind,
 ...
 */
function requireSourceArtifactInputRecord(
  value: unknown,
  code: SourceArtifactAwsFailureCode,
): object {
  if (
    typeof value !== 'object' ||
    value === null ||
    ...
    return failSourceArtifactAws(code)
  }
  return value
}
 ...
 * @param code - Private failure code used for invalid input.
 */
function requireExactSourceArtifactInputKeys(
  value: object,
 ...
 */
function requireSourceArtifactConfiguration(
  configuration: WorkspaceSearchMigrationConfiguration,
): void {
  if (
    ...
    return failSourceArtifactAws('INVALID_ARGUMENT')
  }
  const journal = requireSourceArtifactInputRecord(
  ...
  if (
    !isValidSourceArtifactBucketName(bucketName) ||
    !isMeasuredSourceArtifactKmsKeyArn(
      keyArn,
  ...
  const tables = requireSourceArtifactInputRecord(
    configuration.tables,
    'INVALID_ARGUMENT',
  )
  for (const source of workspaceSearchMigrationSourceNames) {
    requireSourceArtifactTableIdentity(
      Reflect.get(tables, source),
      source,
      configuration,
    )
  }
 ...
 */
function isMeasuredSourceArtifactKmsKeyArn(
  value: unknown,
  region: string,
  account: string,
): value is string {
 ...
 */
function isValidSourceArtifactBucketName(value: unknown): value is string {
  if (
    typeof value !== 'string' ||
    value.length < 3 ||
    value.length > 63 ||
 ...
 * @param configuration - Adapter-bound measured configuration.
 */
function requireSourceArtifactTableIdentity(
  value: unknown,
 ...
 */
function requireSourceArtifactTransport(transport: unknown): void {
  if (
    typeof transport !== 'object' ||
    transport === null ||
    typeof Reflect.get(transport, 'putSourceArtifact') !== 'function' ||
    typeof Reflect.get(transport, 'headSourceArtifact') !== 'function' ||
    typeof Reflect.get(transport, 'getSourceArtifact') !== 'function'
  ) {
    return failSourceArtifactAws('INVALID_ARGUMENT')
  }
}
...
/**
 * Detaches and validates one expected page identity without source items.
 *
 * @param value - Candidate caller-owned page identity.
 * @returns Exact detached expected page identity.
 */
function snapshotExpectedSourceArtifactPageContext(
  value: unknown,
): WorkspaceSearchMigrationPlanningSourceArtifactExpectedPageContext {
  const record = requireSourceArtifactInputRecord(
    value,
  ...
  const source = readSourceArtifactSource(
    Reflect.get(record, 'source'),
  )
  const expected: WorkspaceSearchMigrationPlanningSourceArtifactExpectedPageContext = {
    runId: readSourceArtifactIdentifier(
      Reflect.get(record, 'runId'),
    ),
    configurationHash: readSourceArtifactDigest(
      Reflect.get(record, 'configurationHash'),
      'INVALID_ARGUMENT',
    ),
    source,
    sourceTable: snapshotSourceArtifactTableIncarnation(
      Reflect.get(record, 'sourceTable'),
      'INVALID_ARGUMENT',
    ),
    stateTable: snapshotSourceArtifactTableIncarnation(
      Reflect.get(record, 'stateTable'),
      'INVALID_ARGUMENT',
    ),
    pageSequence: readSourceArtifactPositiveSafeInteger(
      Reflect.get(record, 'pageSequence'),
      'INVALID_ARGUMENT',
    ),
    previousEvidenceDigest: readSourceArtifactDigest(
      Reflect.get(record, 'previousEvidenceDigest'),
      'INVALID_ARGUMENT',
    ),
    previousCheckpointDigest: readSourceArtifactDigest(
      Reflect.get(record, 'previousCheckpointDigest'),
      'INVALID_ARGUMENT',
    ),
    planningAuthority: snapshotSourceArtifactPlanningAuthority(
 ...
 */
function snapshotSourceArtifactTableIncarnation(
  value: unknown,
  code: SourceArtifactAwsFailureCode,
): WorkspaceSearchMigrationPlanningSourceArtifactTableIncarnation {
  const record = requireSourceArtifactInputRecord(value, code)
  requireExactSourceArtifactInputKeys(
    record,
  ...
  return {
    tableName: readSourceArtifactBoundedText(
      Reflect.get(record, 'tableName'),
      255,
      code,
    ),
    tableArn: readSourceArtifactBoundedText(
      Reflect.get(record, 'tableArn'),
      2_048,
      code,
    ),
    tableId: readSourceArtifactBoundedText(
      Reflect.get(record, 'tableId'),
      1_024,
      code,
    ),
    creationTime: readSourceArtifactCanonicalTime(
 ...
 */
function snapshotSourceArtifactPlanningAuthority(
  value: unknown,
  code: SourceArtifactAwsFailureCode,
): WorkspaceSearchMigrationPlanningSourceArtifactAuthority {
  const record = requireSourceArtifactInputRecord(value, code)
  ...
  return {
    ownerId: readSourceArtifactIdentifier(
      Reflect.get(record, 'ownerId'),
      code,
    ),
    fenceToken: readSourceArtifactPositiveSafeInteger(
      Reflect.get(record, 'fenceToken'),
      code,
    ),
    maintenanceEvidencePointerRevision:
      readSourceArtifactPositiveSafeInteger(
        Reflect.get(
          record,
          'maintenanceEvidencePointerRevision',
        ),
    ...
    maintenanceEvidenceReceiptDigest: readSourceArtifactDigest(
...
/**
 * Reads one supported logical source name.
 *
 * @param value - Candidate logical source.
 * @returns Exact supported source name.
 */
function readSourceArtifactSource(
  value: unknown,
 ...
 */
function readSourceArtifactIdentifier(
  value: unknown,
  code: SourceArtifactAwsFailureCode = 'INVALID_ARGUMENT',
): string {
  if (
    typeof value !== 'string' ||
 ...
 */
function readSourceArtifactDigest(
  value: unknown,
  code: SourceArtifactAwsFailureCode,
): string {
  if (!isHexDigest(value)) return failSourceArtifactAws(code)
  return value
}
 ...
 */
function readSourceArtifactPositiveSafeInteger(
  value: unknown,
  code: SourceArtifactAwsFailureCode,
): number {
  if (
 ...
 */
function readSourceArtifactBoundedText(
  value: unknown,
 ...
 */
function readSourceArtifactCanonicalTime(
  value: unknown,
  code: SourceArtifactAwsFailureCode,
): string {
  if (!isCanonicalTimestamp(value)) return failSourceArtifactAws(code)
 ...
 */
function readDenseSourceArtifactArray(
  value: unknown,
  code: SourceArtifactAwsFailureCode,
): readonly unknown[] {
  try {
...
/**
 * Detaches and validates one exact immutable source-artifact reference.
 *
 * @param value - Candidate evidence reference.
 * @returns Exact detached reference.
 */
function snapshotSourceArtifactReference(
  value: unknown,
): WorkspaceSearchMigrationPlanningSourceArtifactReference {
  const record = requireSourceArtifactInputRecord(
    value,
  ...
  const contentDigest = readSourceArtifactDigest(
    Reflect.get(record, 'contentDigest'),
    'INVALID_SOURCE_ARTIFACT',
  )
  const objectKey = readSourceArtifactBoundedText(
    Reflect.get(record, 'objectKey'),
    1_024,
    'INVALID_SOURCE_ARTIFACT',
  )
  const versionId = readSourceArtifactBoundedText(
...
function requireExpectedPageConfigurationBinding(
  expected: WorkspaceSearchMigrationPlanningSourceArtifactExpectedPageContext,
  configuration: WorkspaceSearchMigrationConfiguration,
  configurationHash: string,
): void {
  ...
  const sourceTable = configuration.tables[expected.source]
  const stateTable = configuration.tables['migration-state']
  if (
    sourceTable === undefined ||
    stateTable === undefined ||
    sourceTable.role !== expected.source ||
    stateTable.role !== 'migration-state' ||
    !sourceArtifactTableIncarnationMatches(
    ...
    return failSourceArtifactAws('IDENTITY_MISMATCH')
  }
}
...
function requirePageMatchesExpected(
  ...
  if (
    ...
    !sourceArtifactPlanningAuthorityMatches(
 ...
 */
function sourceArtifactTableIncarnationMatches(
  left: WorkspaceSearchMigrationPlanningSourceArtifactTableIncarnation,
  right:
    | WorkspaceSearchMigrationPlanningSourceArtifactTableIncarnation
 ...
 */
function sourceArtifactPlanningAuthorityMatches(
  left: WorkspaceSearchMigrationPlanningSourceArtifactAuthority,
  right: WorkspaceSearchMigrationPlanningSourceArtifactAuthority,
): boolean {
 ...
 */
function sourceArtifactBytesEqual(
  left: Uint8Array,
  right: Uint8Array,
): boolean {
 ...
 */
function createSourceArtifactChecksumSha256(
  bytes: Uint8Array,
): string {
 ...
 */
function validateSourceArtifactPutOutput(
  ...
  try {
    ...
    const versionId = readOptionalSourceArtifactOutputString(
      Reflect.get(record, 'VersionId'),
    )
    const checksumSha256 = readOptionalSourceArtifactOutputString(
      Reflect.get(record, 'ChecksumSHA256'),
    )
    const checksumType = readOptionalSourceArtifactOutputString(
      Reflect.get(record, 'ChecksumType'),
    )
    const serverSideEncryption =
      readOptionalSourceArtifactOutputString(
        Reflect.get(record, 'ServerSideEncryption'),
      )
    const sseKmsKeyId = readOptionalSourceArtifactOutputString(
      Reflect.get(record, 'SSEKMSKeyId'),
    )
    const bucketKeyEnabled =
      readOptionalSourceArtifactOutputBoolean(
        Reflect.get(record, 'BucketKeyEnabled'),
      )
    const size = readOptionalSourceArtifactOutputNumber(
 ...
 */
function readOptionalSourceArtifactOutputString(
  value: unknown,
): string | undefined {
 ...
 */
function readOptionalSourceArtifactOutputBoolean(
  value: unknown,
): boolean | undefined {
  if (value === undefined) return undefined
 ...
 */
function readOptionalSourceArtifactOutputNumber(
  value: unknown,
): number | undefined {
  if (value === undefined) return undefined
  if (
 ...
 */
function snapshotSourceArtifactObjectOutput(
  output: HeadObjectCommandOutput | GetObjectCommandOutput,
  includeBody: boolean,
): SourceArtifactObjectSnapshot {
  try {
    const record = requireSourceArtifactInputRecord(
      output,
      'INVALID_SOURCE_ARTIFACT',
    ...
    return {
      versionId: readOptionalSourceArtifactOutputString(
        Reflect.get(record, 'VersionId'),
      ),
      contentLength: readOptionalSourceArtifactOutputNumber(
        Reflect.get(record, 'ContentLength'),
      ),
      contentType: readOptionalSourceArtifactOutputString(
        Reflect.get(record, 'ContentType'),
      ),
      checksumSha256: readOptionalSourceArtifactOutputString(
        Reflect.get(record, 'ChecksumSHA256'),
      ),
      checksumType: readOptionalSourceArtifactOutputString(
        Reflect.get(record, 'ChecksumType'),
      ),
      ...
      sseKmsKeyId: readOptionalSourceArtifactOutputString(
        Reflect.get(record, 'SSEKMSKeyId'),
      ),
      bucketKeyEnabled:
        readOptionalSourceArtifactOutputBoolean(
      ...
      deleteMarker: readOptionalSourceArtifactOutputBoolean(
        Reflect.get(record, 'DeleteMarker'),
      ),
      lastModifiedEpochMilliseconds:
        readOptionalSourceArtifactOutputDate(
          Reflect.get(record, 'LastModified'),
        ),
      objectLockMode: readOptionalSourceArtifactOutputString(
      ...
      metadata: snapshotSourceArtifactObjectMetadata(
 ...
 */
function readOptionalSourceArtifactOutputDate(
  value: unknown,
): number | undefined {
  if (value === undefined) return undefined
 ...
 */
function snapshotSourceArtifactObjectMetadata(
  value: unknown,
 ...
 */
function validateStoredSourceArtifactObject(
  snapshot: SourceArtifactObjectSnapshot,
  segment: PreparedSourceArtifactSegment,
  configuration: WorkspaceSearchMigrationConfiguration,
  ...
  if (
    ...
    snapshot.contentType !== sourceArtifactContentType ||
    ...
    !sourceArtifactObjectHasMeasuredRetention(
      snapshot,
      configuration,
    ) ||
 ...
 */
function createSourceArtifactReference(
  segment: PreparedSourceArtifactSegment,
  versionId: string,
): WorkspaceSearchMigrationPlanningSourceArtifactReference {
  return {
 ...
 */
function validateReferencedSourceArtifactObject(
  snapshot: SourceArtifactObjectSnapshot,
  reference: WorkspaceSearchMigrationPlanningSourceArtifactReference,
  segmentIndex: number,
  segmentCount: number,
  ...
  const expectedMetadata = createSourceArtifactMetadata(
  ...
  if (
    ...
    snapshot.checksumSha256 !==
      createSourceArtifactChecksumFromDigest(reference.contentDigest) ||
    (snapshot.checksumType !== undefined &&
      snapshot.checksumType !== 'FULL_OBJECT') ||
    snapshot.serverSideEncryption !== 'aws:kms' ||
    snapshot.sseKmsKeyId !== configuration.journal.keyArn ||
 ...
 */
function sourceArtifactObjectHasMeasuredRetention(
  snapshot: SourceArtifactObjectSnapshot,
  configuration: WorkspaceSearchMigrationConfiguration,
): boolean {
 ...
 */
function createSourceArtifactChecksumFromDigest(
  contentDigest: string,
): string {
 ...
 */
function sourceArtifactMetadataMatches(
 ...
 */
async function readBoundedSourceArtifactBody(
  body: unknown,
  ...
  try {
    ...
    const timeoutResult = new Promise<Uint8Array>(
      (_resolve, reject) => {
        timeout = setTimeout(
          () => reject(new SourceArtifactBodyTimeout()),
          sourceArtifactBodyTimeoutMilliseconds,
        )
      },
    )
    try {
      return await Promise.race([
        readBoundedSourceArtifactIterator(
          iterator,
          nextMethod,
          expectedLength,
        ),
      ...
      cancelSourceArtifactBody(iterator, body)
      throw error
    } finally {
 ...
 * @returns Detached exact object bytes.
 */
async function readBoundedSourceArtifactIterator(
  iterator: unknown,
  nextMethod: unknown,
  expectedLength: number,
): Promise<Uint8Array> {
 ...
 */
function cancelSourceArtifactBody(
  iterator: unknown,
  body: unknown,
): void {
  invokeSourceArtifactCancellationMethod(iterator, 'return')
  invokeSourceArtifactCancellationMethod(body, 'destroy')
  invokeSourceArtifactCancellationMethod(body, 'cancel')
}
 ...
 */
function invokeSourceArtifactCancellationMethod(
  target: unknown,
 ...
 */
function digestSourceArtifactBytes(bytes: Uint8Array): string {
 ...
 */
function isSourceArtifactPreconditionFailure(
  error: unknown,
): boolean {
  try {
    return readSourceArtifactErrorName(error) ===
      'PreconditionFailed' ||
      readSourceArtifactHttpStatusCode(error) === 412
  } catch {
    return false
  }
}
 ...
 */
function isSourceArtifactAmbiguousWriteFailure(
  error: unknown,
): boolean {
  try {
    const status = readSourceArtifactHttpStatusCode(error)
    return status === 409 ||
      (status !== undefined && status >= 500) ||
      isSourceArtifactRetryableFailure(error)
  } catch {
    return false
  }
}
 ...
 */
function isSourceArtifactNotFound(error: unknown): boolean {
  try {
    const name = readSourceArtifactErrorName(error)
    return readSourceArtifactHttpStatusCode(error) === 404 ||
      name === 'NoSuchKey' ||
      name === 'NotFound' ||
      name === 'NoSuchVersion'
  } catch {
 ...
 */
function isSourceArtifactRetryableFailure(
  error: unknown,
): boolean {
  try {
    if (!(error instanceof Error)) return false
    const classificationInput =
      createSourceArtifactAwsErrorClassificationInput(error)
 ...
 */
function readSourceArtifactErrorName(
  error: unknown,
): string | undefined {
 ...
 */
function readSourceArtifactHttpStatusCode(
  error: unknown,
): number | undefined {
  if (typeof error !== 'object' || error === null) return undefined
  const metadata: unknown = Reflect.get(error, '$metadata')
  return readSourceArtifactOptionalNumericProperty(
    metadata,
    'httpStatusCode',
  )
}
 ...
 */
function createSourceArtifactAwsErrorClassificationInput(
  error: Error,
  depth = 0,
): SourceArtifactAwsErrorClassificationInput {
  ...
  const httpStatusCode =
    readSourceArtifactOptionalNumericProperty(
      metadataValue,
      'httpStatusCode',
    )
  const throttling =
    readSourceArtifactOptionalBooleanProperty(
      retryableValue,
      'throttling',
    )
  ...
  return {
    ...
    ...(causeValue instanceof Error
      ? {
          cause:
            createSourceArtifactAwsErrorClassificationInput(
              causeValue,
              depth + 1,
            ),
        }
 ...
 */
function readSourceArtifactOptionalNumericProperty(
  value: unknown,
  property: string,
): number | undefined {
 ...
 */
function readSourceArtifactOptionalBooleanProperty(
  value: unknown,
  property: string,
): boolean | undefined {
  if (typeof value !== 'object' || value === null) return undefined
 ...
 */
async function runSourceArtifactAwsBoundary<T>(
  operation: () => Promise<T>,
): Promise<T> {
  try {
    ...
    throw createSourceArtifactAwsBoundaryFailure(
      readSourceArtifactAwsFailureCode(error),
    )
  }
}
 ...
 */
function readSourceArtifactAwsFailureCode(
  error: unknown,
  fallback:
    WorkspaceSearchMigrationFailureCode = 'INVALID_STATE',
): WorkspaceSearchMigrationFailureCode {
  try {
    if (error instanceof SourceArtifactAwsFailure) return error.code
    if (
      error instanceof
        WorkspaceSearchMigrationSourceArtifactError
    ) {
      return 'INVALID_SOURCE_ARTIFACT'
    }
 ...
 */
function failSourceArtifactAws(
  code: SourceArtifactAwsFailureCode,
): never {
  throw new SourceArtifactAwsFailure(code)
}

/**
 * Creates one public fixed-message source-artifact boundary failure.
 *
 * @param code - Stable operator-safe migration failure code.
 * @returns Fresh secret-free public failure.
 */
function createSourceArtifactAwsBoundaryFailure(
  code: WorkspaceSearchMigrationFailureCode,
): WorkspaceSearchMigrationFailure {
  return new WorkspaceSearchMigrationFailure(
    code,
    `Workspace Search source artifact stopped safely (${code}).`,
  )
}
