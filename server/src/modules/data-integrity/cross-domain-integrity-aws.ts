...
/** Strict reusable preparation result for one File Proofing row. */
type PreparedFileRow = {
  /** Whether the row is a canonical File entry or a known auxiliary entry. */
  readonly kind: 'auxiliary' | 'file'
  /** Strict File references whose tombstones have not expired at the check time. */
  readonly nonExpiredReferences: readonly FileIntegrityReference[]
  /** All strict File references, including retained tombstones. */
  readonly references: readonly FileIntegrityReference[]
  /** Exact retained-evidence capacity required before any exact-version read. */
  readonly requiredItemBudget: number
}
...
export async function readCrossDomainIntegrityAwsNormalizedPage(
  ...
  const preparedFileRows = input.target === 'file-proofing'
    ? decodedRows.map((row) => prepareFileRow(row, input.checkedAt))
    : []
  if (input.target === 'file-proofing') {
    let requiredItemBudget = 0
    for (const preparedRow of preparedFileRows) {
      requiredItemBudget += preparedRow.requiredItemBudget
  ...
  for (let rowIndex = 0; rowIndex < page.items.length; rowIndex += 1) {
    ...
    const preparedFileRow = input.target === 'file-proofing'
      ? preparedFileRows[rowIndex]
      : undefined
    if (input.target === 'file-proofing' && preparedFileRow === undefined) {
      throw new CrossDomainIntegrityAwsBridgeFailure('AWS_PAGE_INVALID')
    }
    const normalized = await normalizeRow(
      input.target,
      row,
      rawItem,
      input,
...
/**
 * Validates and parses one File Proofing row before any exact-version I/O begins.
 *
 * @param row - Decoded low-level File Proofing row.
 * @param checkedAt - Canonical timestamp used to classify expired tombstones.
 * @returns Reusable parsed references and the exact retained-evidence reservation.
 */
function prepareFileRow(
  ...
  if (entryType !== 'file') {
    ...
    return {
      kind: 'auxiliary',
      nonExpiredReferences: [],
      references: [],
      requiredItemBudget: 0,
    }
  }
  ...
  return {
    ...
    nonExpiredReferences,
    references,
    requiredItemBudget: references.length + nonExpiredReferences.length * 2,
  }
}
...
async function normalizeFileRow(
  ...
  const preparedRow = input.preparedRow ?? prepareFileRow(input.row, input.checkedAt)
  if (preparedRow.kind === 'auxiliary') return []
  if (
    retainedEvidenceItemCount(input.collection) + preparedRow.requiredItemBudget >
  ...
  for (const reference of preparedRow.nonExpiredReferences) {
    const objectVersionId = reference.objectVersionId
    if (!objectVersionId) continue
