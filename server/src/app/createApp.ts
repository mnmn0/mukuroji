...
import { createDirectoryProjectId } from '../modules/directory'
import {
  auditEventsToNdjson,
  createAuditEvent,
  createAuditEventId,
...
import { isCanonicalWorkItemRecord } from '../modules/work-items/canonical-work-item'
import {
...
import { createEnterpriseCognitoInspectionCache } from '../modules/enterprise-identity/enterprise-cognito-inspection-cache'
import {
...
import { createDynamoDbConnectorSyncPersistenceFromEnvironment } from '../modules/developer-platform/connector-sync-persistence'
import {
  ConnectorSyncEngine,
...
import { evaluateWebhookEnterpriseTeamAccess } from '../modules/developer-platform/webhook-authorization'
import {
...
/**
 * Hono app が利用する application port と adapter の集合です。
 */
export interface AppDependencies {
  /** Cognito authentication と directory 操作を提供します。 */
  cognito: CognitoClient
  /** Dashboard read model を提供します。 */
  dashboardSummary: DashboardSummaryClient
  /** Project task persistence を提供します。 */
  projectTasks: ProjectTasksClient
  /** Team Work Item persistence を提供します。 */
  teamIssues: TeamIssuesClient
  /** Team、Project、member directory persistence を提供します。 */
  projectDirectory: ProjectDirectoryClient
  /** Append-only audit persistence を提供します。 */
  auditEvents: AuditEventsClient
  /** Workspace membership persistence を提供します。 */
  workspaceAccess: WorkspaceAccessClient
  /** Realtime ticket persistence を提供します。 */
  realtimeTickets: RealtimeTicketsClient
  /** Work Item collaboration persistence を提供します。 */
  collaboration: CollaborationClient
  /** File proofing persistence と object access を提供します。 */
  fileProofing: FileProofingClient
  /** Notification persistence を提供します。 */
  notifications: NotificationClient
  /** Workspace search persistence を提供します。 */
  workspaceSearch: WorkspaceSearchClient
  /** Document persistence を提供します。 */
  documents: DocumentClient
  /** Workspace search projection を同期更新するかを示します。 */
  workspaceSearchProjectionEnabled: boolean
  /** Work Item configuration persistence を提供します。 */
  workItemConfigurations: WorkItemConfigurationClient
  /** Automation persistence を提供します。 */
  automation: AutomationClient
  /** Automation inbound webhook secret persistence を提供します。 */
  automationInboundWebhookSecrets: AutomationInboundWebhookSecretStore
  /** Planning persistence を提供します。 */
  planning: PlanningClient
  /** Developer platform persistence を提供します。 */
  developerPlatform: DeveloperPlatformClient
  /** Canonical public Work Item use case を提供します。 */
  publicWorkItems: PublicWorkItemService
  /** Durable Work Item import execution persistence を提供します。 */
  workItemImportExecutions: WorkItemImportExecutionStore
  /** Durable Work Item import source persistence を提供します。 */
  workItemImportSources: WorkItemImportSourceStore
  /** Durable Work Item import queue を提供します。 */
  workItemImportQueue: WorkItemImportQueue
  /** Request intake persistence を提供します。 */
  requestIntake: RequestIntakeClient
  /** Enterprise identity persistence を提供します。 */
  enterpriseIdentity: EnterpriseIdentityClient
  /** Enterprise session assurance persistence を提供します。 */
  enterpriseSessionActivity: EnterpriseSessionActivityClient
  /** Enterprise identity provider metadata と接続を検証します。 */
  enterpriseIdentityProviderConnectionTester: EnterpriseIdentityProviderConnectionTester
  /** Analytics report と snapshot persistence を提供します。 */
...
const createDependencyProxy = appDependencyRuntime.createProxy

const cognito = createDependencyProxy('cognito')
const dashboardSummary = createDependencyProxy('dashboardSummary')
const projectTasks = createDependencyProxy('projectTasks')
const teamIssues = createDependencyProxy('teamIssues')
const projectDirectory = createDependencyProxy('projectDirectory')
const auditEvents = createDependencyProxy('auditEvents')
const workspaceAccess = createDependencyProxy('workspaceAccess')
const realtimeTickets = createDependencyProxy('realtimeTickets')
const collaboration = createDependencyProxy('collaboration')
const fileProofing = createDependencyProxy('fileProofing')
const notifications = createDependencyProxy('notifications')
const workspaceSearch = createDependencyProxy('workspaceSearch')
const documents = createDependencyProxy('documents')
const workItemConfigurations = createDependencyProxy('workItemConfigurations')
const automation = createDependencyProxy('automation')
const automationInboundWebhookSecrets = createDependencyProxy('automationInboundWebhookSecrets')
const planning = createDependencyProxy('planning')
const developerPlatform = createDependencyProxy('developerPlatform')
const publicWorkItems = createDependencyProxy('publicWorkItems')
const workItemImportExecutions = createDependencyProxy('workItemImportExecutions')
const workItemImportSources = createDependencyProxy('workItemImportSources')
const workItemImportQueue = createDependencyProxy('workItemImportQueue')
const requestIntake = createDependencyProxy('requestIntake')
const enterpriseIdentity = createDependencyProxy('enterpriseIdentity')
const enterpriseSessionActivity = createDependencyProxy('enterpriseSessionActivity')
const analytics = createDependencyProxy('analytics')
const enterpriseIdentityProviderConnectionTester: EnterpriseIdentityProviderConnectionTester = (
  provider,
) => requireAppDependencies().enterpriseIdentityProviderConnectionTester(provider)

function isWorkspaceSearchProjectionEnabled() {
  return requireAppDependencies().workspaceSearchProjectionEnabled
}
...
registerCommonMiddleware(routeApp, {
  getAllowedOrigins,
  auditRejectedEnterpriseSecurityMutation,
})
routeApp.route('/', createSystemRouter())
...
async function createAuthenticationResponse(
  ...
  if (
    requireAppDependencies().cognito instanceof FlociCognitoClient &&
...
/**
 * Production API が利用する concrete adapter を生成します。
 */
...
/**
 * Server test が個別 fake を上書きするための isolated dependency set を生成します。
 */
...
/**
 * Immutable dependency set に束縛された Hono app instance を生成します。
 */
export function createApp(dependencies: AppDependencies): Hono {
  return appDependencyRuntime.bindApp(routeApp, dependencies)
}

let productionDependencies: AppDependencies | undefined
let productionApp: Hono | undefined
let productionApiHandler:
  | ReturnType<typeof createApiHandler>
  | undefined

function getProductionDependencies() {
  productionDependencies ??= createProductionAppDependencies()
  return productionDependencies
}

function getProductionApp() {
  productionApp ??= createApp(getProductionDependencies())
  return productionApp
}

function getProductionApiHandler() {
  productionApiHandler ??= createApiHandler(getProductionDependencies())
  return productionApiHandler
}

/**
 * App instance 外で実行する application operation に明示的な依存 context を束縛します。
 */
...
/**
 * Lambda handler、Bun dev server で共有する production Hono app です。
 */
export const app = new Proxy(routeApp, {
  get(_target, property) {
    const production = getProductionApp()
    const value = Reflect.get(production, property)
    return typeof value === 'function' ? value.bind(production) : value
  },
})

/**
 * 指定された app dependency set に束縛された API Lambda handler を生成します。
 */
...
export const handler = (
  ...
  return getProductionApiHandler()(
    event,
    lambdaContext,
  )
}
...
/**
 * API runtime に必要な production-only configuration を起動時に検証します。
 */
...
export async function workItemImportHandler(event: WorkItemImportSqsEvent) {
  ...
  return appDependencyRuntime.runWith(
    dependencies,
    () =>
      processWorkItemImportBatch(
        event,
