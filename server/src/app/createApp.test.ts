...
import { CollaborationError, type CollaborationClient } from '../modules/collaboration/collaboration'
...
function setTestAppDependencies(
  ...
  testAppDependencies = {
    ...
    workspaceSearchProjectionEnabled: dependencies.workspaceSearch
      ? true
      : testAppDependencies.workspaceSearchProjectionEnabled,
  }
...
function runWithTestAppDependencies<Result>(
  ...
  return runWithAppDependencies(testAppDependencies, operation)
}
...
test('serves the same authenticated API contract from Function URL root and /api paths', async () => {
  await withTestEnvironment(
    ...
    async () => {
      ...
      const testHandler = createApiHandler(testAppDependencies)
...
test('isolates dependency sets across concurrently requested app instances', async () => {
  await withTestEnvironment(
    {
      COGNITO_CLIENT_ID: 'isolated-client',
      COGNITO_USER_POOL_ID: 'us-east-1_isolated',
    },
    async () => {
      ...
      try {
        const [firstResponse, secondResponse] = await Promise.all([
          firstApp.request('/api/auth/login', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ email: 'first@example.com', password: 'password' }),
          }),
          secondApp.request('/api/auth/login', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
test('isolates cached Document roles between app dependency sets', async () => {
  configureFakeProjectClients(true)
  const baseDependencies = testAppDependencies
  ...
  const createDocumentApp = (
    ...
    createApp({
      ...
      } as unknown as AppDependencies['projectDirectory'],
    })
