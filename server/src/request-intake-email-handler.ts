...
let requestEmailClient: RequestIntakeClient = createDefaultRequestIntakeClient()

/** Test で email ingestion client を差し替えます。 */
export function configureRequestEmailClientForTest(client: RequestIntakeClient) {
  requestEmailClient = client
}

/** Test 後に標準 email ingestion client へ戻します。 */
export function resetRequestEmailClientForTest() {
  requestEmailClient = createDefaultRequestIntakeClient()
}
...
/** 署名・freshness を検証した envelope だけを Request thread へ追加します。 */
export async function handler(
  event: SignedRequestEmailEvent,
): Promise<RequestRequesterReplyReceipt> {
  const secret = requireEmailSecret(readEnvironment('REQUEST_EMAIL_WEBHOOK_SECRET'))
  validateSignedEmailEvent(event, secret, new Date())
  return requestEmailClient.ingestEmail(event.envelope)
}
