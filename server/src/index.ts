...
function createRequestConversionTransactionUpdate(
  ...
  return {
    Update: {
      ...
      UpdateExpression:
        'SET #status = :converted, #revision = :nextRevision, workItem = :workItem, updatedAt = :updatedAt, capabilities = :capabilities, events = list_append(if_not_exists(events, :emptyEvents), :events)',
      ConditionExpression:
...
function createRequestExternalContext(c: Context): RequestExternalContext {
  ...
  const localForwardedSource = !getEnv('AWS_LAMBDA_FUNCTION_NAME')
    ? c.req.header('X-Forwarded-For')?.split(',')[0]?.trim()
    : undefined
  const source = trustedSource || localForwardedSource || 'transport-unavailable'
  return {
    clientKey: source,
...
export class DynamoDbTeamIssuesClient {
  ...
  async createTeamIssue(
    ...
    try {
      ...
      const requestConversionUpdate = requestConversion
        ? createRequestConversionTransactionUpdate(
            requestConversion,
            teamId,
            issueId,
      ...
      await this.documentClient.send(
        new TransactWriteCommand({
          TransactItems: [
            ...
            ...(requestConversionUpdate ? [requestConversionUpdate] : []),
          ],
        }),
      )
