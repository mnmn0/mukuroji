...
この graph は意味上の relation を所有します。`blocks` / `blockedBy` も blocker の意味と検索・表示を表すだけで、Work Item の日付を暗黙に移動しません。Cross-Team / cross-Project の日程制約、lead / lag、constraint、cascade preview は PlanningTable の canonical Work Item schedule dependency が所有します。
