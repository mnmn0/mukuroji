...
- required、length、range、安全性を検査した pattern
...
DynamoDB の 400 KB item 上限より十分小さく保つため、normalized form draft は128 KiB、回答全体は96 KiB、thread message history は64 KiB、event history は32 KiB、保存 item は360 KiBを上限とする。件数上限だけでなく UTF-8 byte 数を server で検証し、超過時は DynamoDB へ書き込む前に `413 RequestPayloadTooLarge` を返す。
