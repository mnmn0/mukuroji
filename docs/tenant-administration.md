...
Closure requires `export`, `revoke-access`, `anonymize-members`, `delete-data`, `delete-secrets`, and `verify`. Its export step performs the same snapshot, preparation, and artifact verification phases before deletion starts. Access revocation globally signs out every non-requesting member and removes tenant realtime/HTTP sessions. Member anonymization deletes outstanding invitations, replaces non-requesting member identities with deterministic non-deliverable tombstones while preserving role and lifecycle history, and pseudonymizes member fields in retained tenant control-plane history. Business-data deletion covers each deployed tenant table plus every S3 object version and delete marker under the exact `workspaces/<workspaceId>/` prefix. Credential-bearing Developer Platform and Enterprise Identity rows are deleted by a separate owner. Final verification strongly checks every deletion target and the complete versioned file prefix before it can commit evidence.
