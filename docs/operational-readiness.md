...
fail-closed で拒否します。これは AWS から material を取得する managed composition や application
writer から隔離された snapshot を提供するものではありません。
...
maintenance-window 内の再生成用途に限定します。未実装項目には managed AWS exact-version
material composition、distinct historical receipt の実在・run/fence binding、plan 保存直前の
current lease/pointer/receipt freshness、5 evidence head と plan provenance/storage の原子的な結合、
