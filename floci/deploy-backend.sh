...
FUNCTION_ENV="Variables={MUKUROJI_DASHBOARD_TABLE=$DASHBOARD_TABLE,MUKUROJI_PROJECT_TASKS_TABLE=$PROJECT_TASKS_TABLE,MUKUROJI_PROJECT_DIRECTORY_TABLE=$PROJECT_DIRECTORY_TABLE,MUKUROJI_TEAM_ISSUES_TABLE=$TEAM_ISSUES_TABLE,MUKUROJI_TEAM_ISSUE_EVENTS_TABLE=$TEAM_ISSUE_EVENTS_TABLE,MUKUROJI_COLLABORATION_TABLE=$COLLABORATION_TABLE,COLLABORATION_TABLE_NAME=$COLLABORATION_TABLE,MUKUROJI_WORKSPACE_SEARCH_TABLE=$WORKSPACE_SEARCH_TABLE,WORKSPACE_SEARCH_TABLE_NAME=$WORKSPACE_SEARCH_TABLE,MUKUROJI_NOTIFICATIONS_TABLE=$NOTIFICATIONS_TABLE,NOTIFICATIONS_TABLE_NAME=$NOTIFICATIONS_TABLE,MUKUROJI_REALTIME_SESSIONS_TABLE=$REALTIME_SESSIONS_TABLE,REALTIME_SESSIONS_TABLE_NAME=$REALTIME_SESSIONS_TABLE,MUKUROJI_WORKSPACE_DIRECTORY_ID=$WORKSPACE_DIRECTORY_ID,MUKUROJI_PROJECT_DIRECTORY_ID=$WORKSPACE_DIRECTORY_ID,MUKUROJI_WORKSPACE_ACCESS_TABLE=$WORKSPACE_ACCESS_TABLE,WORKSPACE_ACCESS_TABLE_NAME=$WORKSPACE_ACCESS_TABLE,AUDIT_EVENTS_TABLE_NAME=$AUDIT_EVENTS_TABLE,AUDIT_RETENTION_DAYS=$AUDIT_RETENTION_DAYS,MUKUROJI_AUTOMATION_TABLE=$AUTOMATION_TABLE,AUTOMATION_TABLE_NAME=$AUTOMATION_TABLE,AUTOMATION_INBOUND_WEBHOOK_SECRET_PREFIX=$AUTOMATION_INBOUND_WEBHOOK_SECRET_PREFIX,AUTOMATION_INBOUND_WEBHOOK_BASE_URL=$API_BASE_URL,SECRETS_MANAGER_ENDPOINT=$LAMBDA_SECRETS_MANAGER_ENDPOINT,AWS_ENDPOINT_URL_SECRETSMANAGER=$LAMBDA_SECRETS_MANAGER_ENDPOINT,COGNITO_USER_POOL_ID=$POOL_ID,COGNITO_ISSUER=$COGNITO_ISSUER,COGNITO_ENDPOINT=$LAMBDA_FLOCI_ENDPOINT,DYNAMODB_ENDPOINT=$LAMBDA_FLOCI_ENDPOINT,AWS_REGION=$AWS_REGION"
if [ -n "$CLIENT_ID" ]; then
