...
FUNCTION_ENV="Variables={MUKUROJI_DASHBOARD_TABLE=$DASHBOARD_TABLE,MUKUROJI_PROJECT_TASKS_TABLE=$PROJECT_TASKS_TABLE,MUKUROJI_PROJECT_DIRECTORY_TABLE=$PROJECT_DIRECTORY_TABLE,MUKUROJI_WORKSPACE_DIRECTORY_ID=$WORKSPACE_DIRECTORY_ID,MUKUROJI_PROJECT_DIRECTORY_ID=$WORKSPACE_DIRECTORY_ID,MUKUROJI_WORKSPACE_ACCESS_TABLE=$WORKSPACE_ACCESS_TABLE,WORKSPACE_ACCESS_TABLE_NAME=$WORKSPACE_ACCESS_TABLE,AUDIT_EVENTS_TABLE_NAME=$AUDIT_EVENTS_TABLE,AUDIT_RETENTION_DAYS=$AUDIT_RETENTION_DAYS,COGNITO_USER_POOL_ID=$POOL_ID,COGNITO_ISSUER=$COGNITO_ISSUER,COGNITO_ENDPOINT=$LAMBDA_FLOCI_ENDPOINT,DYNAMODB_ENDPOINT=$LAMBDA_FLOCI_ENDPOINT,AWS_REGION=$AWS_REGION"
if [ -n "$CLIENT_ID" ]; then
