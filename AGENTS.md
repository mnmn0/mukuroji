...
- 同じPRの GitHub Actions Checks で、各workflowの `on` に定義された全trigger条件（event名、eventごとの `types`、`branches`、`branches-ignore`、`paths`、`paths-ignore`、その他のevent固有filter）と対象commitの変更ファイル・workflow pathから適用対象を判定し、適用対象のworkflowだけが `completed` / `success` になることを確認する。`pull_request_target`、`push`、`workflow_run` などeventごとの条件を省略して適用対象を判定してはならない。path-filtered workflow が対象外で未実行または意図的に `skipped` の場合は、workflow path、event、対象commitのfull SHA、変更ファイル、全trigger条件の判定、GitHub上の未実行 / `skipped` 結果をrelease evidenceとして記録する。適用対象の `in_progress`、`queued`、`skipped`、`failure`、`cancelled`、`timed_out` は完了扱いにしない。
