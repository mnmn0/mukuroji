...
/** Configuration returned after an external Work Item Type update removes Activity. */
const activityRemovedFromCurrentTypeConfiguration = {
  ...activityOptionalTypeConfiguration,
  workItemTypes: activityOptionalTypeConfiguration.workItemTypes.map((type) =>
    type.id === DEFAULT_WORK_ITEM_TYPE.id
      ? {
          ...type,
          detailSections: type.detailSections.filter((section) => section !== 'activity'),
        }
      : type,
  ),
}
...
/** Props for the external Work Item Type revalidation regression harness. */
type ExternalTypeRevalidationHarnessProps = {
  /** Task detail inputs rendered before and after the simulated external update. */
  taskDetailPaneProps: TaskDetailPaneProps
}

/** Simulates a newer Work Item revision whose type no longer exposes Activity. */
function ExternalTypeRevalidationHarness({
  taskDetailPaneProps,
}: ExternalTypeRevalidationHarnessProps) {
  const [isExternalTypeUpdateApplied, setIsExternalTypeUpdateApplied] = useState(false)
  const initialDetail = taskDetailPaneProps.detail
  if (!initialDetail) throw new globalThis.Error('Expected a selected Work Item detail.')
  const refreshedDetail = isExternalTypeUpdateApplied
    ? {
        ...initialDetail,
        issue: {
          ...initialDetail.issue,
          revision: initialDetail.issue.revision + 1,
          workItemTypeId: 'brief',
        },
        resolvedConfiguration: {
          configuration: activityRemovedFromCurrentTypeConfiguration,
        },
      }
    : initialDetail
  ...
  return (
    <>
      <button
        data-testid="simulate-external-type-revalidation"
        onClick={() => setIsExternalTypeUpdateApplied(true)}
        type="button"
      >
        Simulate external type update
      </button>
      <TaskDetailPane {...taskDetailPaneProps} detail={refreshedDetail} />
    </>
  )
}
...
/** Retains a dirty comment as read-only when external revalidation removes Activity. */
export const ExternalTypeRefreshRemovingActivityRetainsCommentDraft: Story = {
  args: {
    collaboration: issueCollaborationControllerFixture,
    configuration: activityOptionalTypeConfiguration,
    detail: activityOptionalTypeDetail,
  ...
  render: ({ showAiAssistance = false, ...args }) => (
    <ExternalTypeRevalidationHarness
      taskDetailPaneProps={{
        ...args,
        renderAiAssistance: createStoryAiAssistanceRenderer(showAiAssistance),
      }}
  ...
  play: async ({ canvasElement }) => {
    ...
    await userEvent.type(commentBody(), '外部の種別更新後も保持するコメント')
    await userEvent.click(canvas.getByTestId('simulate-external-type-revalidation'))
