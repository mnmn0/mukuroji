...
export function PlanningPage() {
  ...
  const currentUserKey = accessToken ? ['planning-current-user', accessToken] as const : null
  const {
    data: user,
  ...
  const projectDirectoryKey = accessToken && user && !currentUserError
    ? ['planning-project-directory', accessToken, locale] as const
    : null
  const {
  ...
  const planningKey = accessToken && user && !currentUserError
  ...
  const planningProjectRolesKey =
    accessToken && user && !currentUserError && !isProjectDirectoryLoading &&
      !user.isSystemAdmin && canMutateWorkspaceContent(user)
  ...
  const {
    ...
    ([, token, memberKey, currentProjectIds]) =>
...
function resolveCurrentUserProjectKey(user: Awaited<ReturnType<typeof getCurrentUser>> | undefined) {
  return (user?.attributes.email ?? user?.username ?? '').trim().toLowerCase()
}

async function loadPlanningProjectRoles(
  accessToken: string,
  memberKey: string,
