...
/**
 * Renders the searchable and role-filtered Team member directory.
 *
 * @param props - Team member data, Project navigation, and localized labels.
 * @returns The pure Team member directory view.
 */
export function TeamMembersView({
  ...
  const members = createTeamMemberRows(
    projects,
    tasks,
    teamProjectMembers,
    referenceDate,
  ...
  return (
    <div className="grid gap-6">
      <div className="grid grid-cols-4 gap-4 max-[1180px]:grid-cols-2 max-[680px]:grid-cols-1">
        ...
        />
        <MetricCard
          label={t('workspace.members.metric.managers')}
          value={managerCount}
          tone="amber"
        />
        <MetricCard
        ...
        />
        <MetricCard
          label={t('workspace.members.metric.open')}
          value={openTaskCount}
          tone="emerald"
        />
      </div>
            ...
            <label className="grid gap-2 text-xs font-semibold uppercase tracking-[0.08em] text-[var(--workbench-muted)]">
              ...
              <select
                ...
                onChange={(event) => {
                  ...
                  if (isTeamMemberRoleFilter(value)) {
                    setRoleFilter(value)
                  }
                }}
              >
                ...
                <option value="manager">
                  {t('workspace.permissions.role.manager')}
                </option>
          ...
          {filteredMembers.map((member) => (
            ...
            >
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="workbench-badge">
                    {member.role
                      ? t(`workspace.permissions.role.${member.role}`)
                      : t('workspace.members.role.none')}
                  </span>
                </div>
              ...
              <div className="grid gap-2 text-sm font-bold leading-6 text-[#526381]">
                <p>
                  {t('workspace.members.taskCount').replace(
                    '{count}',
                    String(member.taskCount),
                  )}
                </p>
                <div className="flex flex-wrap gap-2">
                  <span className="workbench-badge">
                    {t('workspace.members.openTaskCount').replace(
                      '{count}',
                      String(member.openTaskCount),
                    )}
                  </span>
                  <span className="workbench-badge-warning">
                    {t('workspace.members.reviewTaskCount').replace(
                      '{count}',
                      String(member.reviewTaskCount),
                    )}
                  </span>
                  <span
                    className={member.attentionTaskCount > 0
                      ? 'workbench-badge-danger'
                      : 'workbench-badge-success'}
                  >
                    {t('workspace.members.attentionTaskCount').replace(
                      '{count}',
                      String(member.attentionTaskCount),
                    )}
                  </span>
                </div>
              </div>
