...
export function Sidebar({
  ...
  return (
    <aside
      className={`flex h-screen flex-none flex-col overflow-hidden bg-[#03172f] py-5 text-white shadow-[18px_0_38px_rgba(5,23,48,0.18)] transition-all duration-200 ${isCollapsed ? 'w-[76px] px-3' : 'w-[292px] px-4'} ${className}`}
      aria-label={resolvedLabels.ariaLabel}
      data-collapsed={isCollapsed}
    >
      <div
        className={`mb-5 flex items-center ${isCollapsed ? 'flex-col gap-3 px-0' : 'justify-between px-1'}`}
      >
      ...
      {!isCollapsed ? (
        <div className="mt-6 flex items-center justify-between px-1 text-app-meta font-semibold text-slate-200">
      ...
      <nav className="mt-3 space-y-1 border-t border-white/10 pt-3" aria-label={resolvedLabels.utilityNavigation}>
